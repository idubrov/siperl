...
setup() ->
    ...
    Send = fun (Conn, Msg) ->
                    ...
                    TestPid ! {tp, Conn, {Kind, Msg}},
                    {ok, TestPid}
                  end,
    meck:expect(sip_transport, send, Send),
    {Pid}.
...
client_invite_ok(Transport) ->
    ...
    {ok, TxRef} = sip_transaction:start_client_tx(self(), To, Request),
...
server_invite_ok(Transport) ->
    ...
    % Request is received
    {ok, TxRef} = sip_transaction:start_server_tx(self(), Connection, Request),
...
server_invite_tu_down(Transport) ->
    ...
    % Request is received
    {ok, TxRef} = sip_transaction:start_server_tx(TU, Connection, Request),
