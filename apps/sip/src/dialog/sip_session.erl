...
%% @private
-spec handle_call
        ({offer, sip_session_id(), local | remote, sip_session_desc()}, gen_from(), #state{}) ->
            {reply, ok, #state{}}
          | {reply, {error, already_offered}, #state{}};
        ({answer, sip_session_id(), local | remote, sip_session_desc()}, gen_from(), #state{}) ->
            {reply, ok, #state{}}
          | {reply, {error, no_session}, #state{}}
          | {reply, {error, not_offered}, #state{}}
          | {reply, {error, offered_same_side}, #state{}};
        ({lookup, sip_session_id()}, gen_from(), #state{}) ->
            {reply, {ok, #sip_session{}}}
          | {reply, {error, not_offered}, #state{}}.
handle_call({offer, SessionId, Side, SessionDesc}, _From, State) ->
    case lookup_create(SessionId, State) of
        % only set offer if not offered yet, otherwise ignore
